...
int intersection(int setA[SIZEA], int setB[SIZEB], int result[max(SIZEA, SIZEB)]){
    int i, j, k, m;
    int copyA[SIZEA];
    for (i = 0 ; i < SIZEA ; i++){
        copyA[i] = setA[i];
...
int diffrence(int setA[SIZEA], int setB[SIZEB], int result[max(SIZEA, SIZEB)]){  // A \ B
    int i, j, k;
    int copyB[SIZEB];
    for (i = 0 ; i < SIZEB ; i++){
        copyB[i] = setB[i];
...
int symmetricDiffrence(int setA[SIZEA], int setB[SIZEB], int result[max(SIZEA, SIZEB)]){ // A - B
    int i, j, k, n, m;
    int copyA[SIZEA];
    int copyB[SIZEB];
    int repetitions = 0;
...
int main(){
    ...
    while ((scanf("%d;%d,%d,%d,%d;%d,%d,%d,%d,%d,%d",&n,&setA[0],&setA[1],&setA[2],&setA[3],&setB[0],&setB[1],&setB[2],&setB[3],&setB[4],&setB[5])) != NULL){
        if ( n == 1){
